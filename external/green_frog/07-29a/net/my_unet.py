...
def make_conv_bn_relu(in_channels, out_channels,
...
class UNet256(nn.Module):
    def __init__(self, in_shape, num_classes):
        ...
        self.classify = nn.Conv2d(32, num_classes,
...
if __name__ == '__main__':
    ...
    if 0:  # CrossEntropyLoss2d()
        ...
        net = UNet256_1(in_shape=(C, H, W), num_classes=2).cuda().train()
    ...
    if 1:  # BCELoss2d()
        ...
        net = UNet256_1(in_shape=(C,H,W), num_classes=1).cuda().train()
        x = Variable(inputs).cuda()
        y = Variable(labels).cuda()
        logits = net.forward(x)
