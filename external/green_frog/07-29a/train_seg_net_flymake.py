...
from net.segmentation.my_unet import BCELoss2d, UNet256_1 as Net
...
def run_train():
    ...
    out_dir  = '/root/share/project/kaggle-carvana-cars/results/xx5'
    ...
    batch_size = 32
    train_dataset = KgCarDataset( 'train256x256_5088',  #'train_5088'
                                transform=[
    ...
    #todo: add valid_loader (validatation dataset)
    valid_dataset = None



    test_dataset = KgCarDataset( 'test256x256_3197',
                                  is_label=False,
                                  is_preload=False,)
    ...
    for epoch in range(start_epoch, num_epoches):  # loop over the dataset multiple times
        ...
        for it, (images, labels, indices) in enumerate(train_loader, 0):
            ...
            if it%it_print == 0 or it==num_its-1:
                ...
                train_acc  = dice_loss(masks, labels).data[0]
                train_loss = loss.data[0]

                print('\r%5.1f   %5d    %0.4f   |  %0.4f  | %0.4f  %6.4f | ... ' % \
                        (epoch + (it+1)/num_its, it+1, rate, smooth_loss, train_loss, train_acc),\
                        end='',flush=True)
